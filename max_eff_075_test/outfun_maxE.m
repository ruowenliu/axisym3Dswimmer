...
switch state
    case 'iter'
        if optimValues.iteration ~=0
            ...
            figK = figK+1;
            saveas(gcf, ['./maxeff_nu_' num2str(nu*100,'%.3i') 'iteration_' num2str(figK) '.png']);
            close all

            plot(shape_current.arclen,shape_current.uslip)
            saveas(gcf, ['./maxeff_nu_' num2str(nu*100,'%.3i') 'iteration_' num2str(figK) '_uslip.png']);
            close all

            fID = fopen(['./maxeff_nu_' num2str(nu*100,'%.3i') 'designvec_iteration_' num2str(figK) '.txt'], 'w');
            fprintf(fID, '%.15f \n', design_vec);
            fclose(fID);
