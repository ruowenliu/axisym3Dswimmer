function [J, grad] = objectiveMaxEffi(design_vec, THETA_bas, c, compute_grad)
...
if (min(real(shape.x))>0) && (max(abs(shape.cur))<1e10) && (shape.JE>0) % check if x goes negative or curvature too large (very sharp tip) or JE is positive
    J = - shape.JE - c.lam*Cnu + c.sig/2*(Cnu^2);
else
    J = Inf;
end
...
if strcmp(compute_grad,'grad on')
    ...
    for k = 1:num_grad
        shapeDer = shape_derivatives(shape, THETA_bas(:,k));
        derivCnu = c.multi_cst*shapeDer.rvolprime;
        grad(k) = - shapeDer.JEprime - c.lam*derivCnu + c.sig*Cnu*derivCnu;
    end
else
    grad = [];
end
