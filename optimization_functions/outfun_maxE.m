...
switch state
    case 'iter'
        if optimValues.iteration ~=0
            ...
            figK = figK+1;
            saveas(gcf, [dir_result 'maxE_nu_' num2str(nu, '%.2e') '_shape_iter_' num2str(figK) '.png']);
            close all

            plot(shape_current.arclen,shape_current.uslip)
            saveas(gcf, [dir_result 'maxE_nu_' num2str(nu, '%.2e') '_uslip_iter_' num2str(figK) '.png']);
            close all

            fID = fopen([dir_result 'maxE_nu_' num2str(nu, '%.2e') '_designvec_iter_' num2str(figK) '.txt'], 'w');
            fprintf(fID, '%.15f \n', design_vec);
            fclose(fID);
