...
for row = 1:size(input_data,1)
    ...
    diary_name = ['min_drag_force_diary_nu_' num2str(nu, '%.2e') '.txt'];
    ...
    saveas(gcf, ['./iteration_0_nu_' num2str(nu, '%.2e') '_uslip.png']);
    close all
    ...
    saveas(gcf, ['./iteration_final_nu_' num2str(nu, '%.2e') '_uslip.png']);
    close all

    fID = fopen(['./final_designvec_mindragforce_nu_' num2str(nu, '%.2e') '.txt'], 'w');
    fprintf(fID, '%.15f \n', design_vec);
    fclose(fID);
